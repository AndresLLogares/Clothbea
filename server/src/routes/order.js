...
OrderRoute.post("/createorder", async (req, res) => {
  ...
  await Order.findOne({ Id: Id }).then((order) => {
    if (order) {
...
OrderRoute.post("/orderbyuser", async (req, res) => {
  ...
  await Order.findOne({ email: email }).then((order) => {
    if (!order) {
      return res.send("order doesn't exist");
    } else {
      res.json(order);
...
OrderRoute.post("/paymentcheckout", async (req, res) => {
  ...
  await Order.findOne({ Id: Id }).then(async (order) => {
    if (!order) {
      ...
      if (response.status === "succeeded") {
        ...
        while (count < products.length) {
          await Products.findOne({ Id: products[count].Id }).then((product) => {
