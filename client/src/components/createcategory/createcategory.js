...
const CreateCategory = () => {
  ...
  const handleSubmitChange = async (event) => {
    ...
    if (!checkMan && !checkWoman) {
      return toast.error("You must choice a genre");
    } else if (!checkWoman && checkMan) {
      setGenre(["Men"]);
    } else if (checkWoman && !checkMan) {
      setGenre(["Women"]);
    }
    await axios
      .post(URL + "/Products/createcategories", {
        Id: Id + 1,
        name: category,
        category: genre,
      })
              ...
              <form onSubmit={handleSubmitChange} className={styles.form}>
                <div className={styles.eachInput}>
                  <label className={styles.label}>
                    <CategoryAlt className={styles.iconsLogin} /> Women
                  </label>
                  <input
                ...
                <div className={styles.eachInput}>
                  <label className={styles.label}>
                    <CategoryAlt className={styles.iconsLogin} /> Men
                  </label>
                  <input
                ...
                <div className={styles.eachInput}>
                  <label class="input">
                    ...
                    <span class="input__label">
                      <Category className={styles.iconsLogin} /> Category
                    </span>
                  </label>
                </div>
