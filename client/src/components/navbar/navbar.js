      ...
      <div className={styles.PContainer}>
        <Link style={{ textDecoration: "none" }} to="/">
          <div className={styles.PLink}>
            <img className={styles.logoNav} src={Logo} alt="" /> Clothbea
          </div>
        </Link>
      </div>
      <ul className={click ? styles.nav_options_active : styles.nav_options}>
        <li className={styles.SortNav} onClick={closeMobileMenu}>
          ...
          >
            <div className={styles.Links}>
              <Cart3 className={styles.iconCart} />
              {typeof numberCart !== Array ? numberCart.length : 0}
            </div>
          </Link>
        </li>
        <li className={styles.SortNav} onClick={closeMobileMenu}>
        ...
        {nameUser ? (
          <li className={styles.SortNav}>
            <div onClick={handleDrop} className={styles.LinksDrop}>
              {nameUser}
            </div>
            {changeDrop ? <DropDown /> : null}
          </li>
        ) : (
          <li className={styles.SortNav} onClick={closeMobileMenu}>
            <Link
